	...
	var createPageHtml = '\
		<div class="container">\
			<div class="row">\
				...
				<div class="col-xs-7">\
					<button id="pubfriswitch" class="btn btn-sm btn-default" style="width:100%;" onclick="switchPF()">PUBLIC</button>\
				</div>\
			</div>\
					...
					<div class="row">\
						<div class="col-xs-12">\
							<select id="category" class="form-control input-sm">\
								...
								<option value="Healthy Care">Health Care</option>\
			...
			<div class="row mytopbuffer myLocationPicker">\
				<div class="col-xs-12">\
					<button id="choseLocationBtn" onclick="chooseAddress()" class="btn btn-sm btn-default" style="width:100%;">Click to choose location</button>\
					<div class="sr-only" id="myCor"></div><div class="sr-only" id="myAddress"></div>\
				</div>\
			</div>\
			<div class="row mytopbuffer">\
				<div class="col-xs-12">\
...
function switchPF(){
	var tempC = $("#pubfriswitch").html();
	if(tempC == "PUBLIC"){
		$("#pubfriswitch").html("FRIEND");
	}
	if(tempC == "FRIEND"){
		$("#pubfriswitch").html("PUBLIC");
	}
}
...
function initialEvent(){
	var r = confirm("Do you wish to create this event?");
	
	if(r){
		var userId = window.localStorage.getItem("userId");
		
		var publish = $("#pubfriswitch").html();
		
		publish = publish.toLowerCase();
		...
		var cor = $("#myCor").html();
		var address = $("#myAddress").html();
		
		var location = cor + ":" + address;
		...
		if(eventTitle.length==0||category.length==0||duration.length==0||description.length==0||location.length==0){
			alert("please do not leave blanks:)");
		}else{
			var createInput = '{"title":"'+eventTitle+'","description":"'+description+'","startTime":"'+startTime+'","duration":"'+duration+'","tag":"'+category+'","peopleLimit":"'+capacityValue+'","location":"'+location+'","organizer":"'+userId+'","publish":"'+publish+'","image":"'+image+'"}';
						...
						<div class="container">\
							...
							<div class="row">\
								<div class="col-xs-12">'
									...
									'<div class="thumbnail">\
										<div class="row">\
											<div class="col-xs-9 col-sm-10"><input id="newcomment" type="text" class="form-control input-sm"></div>\
											<div class="col-xs-3 col-sm-2 myleftpadding"><button class="btn btn-primary btn-sm" onclick="postMNewComment('+id+')">Post</button></div>\
										</div>\
									</div>\
								</div>\
							</div>\
...
function postMNewComment(eventId){
	var newcomment = $("#newcomment").val();
	postCommentinCreate(eventId, newcomment);
}
							...
							<div class="row">\
								<div class="col-xs-12">'
									...
									'<div class="thumbnail">\
										<div class="row">\
											<div class="col-xs-9 col-sm-10"><input id="newcomment" type="text" class="form-control input-sm"></div>\
											<div class="col-xs-3 col-sm-2 myleftpadding"><button onclick="postJNewComment('+id+')" class="btn btn-primary btn-sm">Post</button></div>\
										</div>\
									</div>\
								</div>\
							</div>\
...
function postJNewComment(eventId){
	var newcomment = $("#newcomment").val();
	postCommentinView(eventId, newcomment);
}
...
function confirmUpdate(id){
	$(document).ready(function(){
		...
		updateJson = encodeURIComponent(updateJson);
		$.ajax({
...
function cancelEvent(id){
	...
	if(r == true){
		...
		$(document).ready(function(){
			$.ajax({
				url: '/ShopperWeb/CancelEventServlet?json='+cancelJson,
				type: 'POST',
				dataType: 'json',
				error: function(err){
					alert("cancel event ajax error!");
				},
				success: function(data){
					console.log(data);
					var status = data["status"];
